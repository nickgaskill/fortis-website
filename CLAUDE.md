...
Plain static HTML/CSS/JS — no framework, no build step. Ten real pages, each a full standalone page: `index`, `services` (an overview hub), the four service-category pages (`interior-remodels`, `exterior-living`, `home-additions-sunrooms`, `new-construction`), plus `gallery`, `process`, `reviews`, `contact`.
