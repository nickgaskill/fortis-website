...
Plain static HTML/CSS/JS — no framework, no build step. Sixteen real pages, each a full standalone page: `index`, `services` (an overview hub), the four service-category pages (`interior-remodels`, `exterior-living`, `home-additions-sunrooms`, `new-construction`), plus `gallery`, `process`, `reviews`, `contact`.
...
- The **header and footer are copy-pasted into all sixteen pages** — if you edit one (add a page link, change the phone number), make the same edit in every HTML file. The `aria-current="page"` marker is set on the current page's nav links.
